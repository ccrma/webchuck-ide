...
export default class Recorder {
    ...
    constructor(recordButton: HTMLButtonElement) {
        ...
        Recorder.removeButton.addEventListener("click", () => {
            if (Recorder.state === RecordState.recording && VmMonitor.getNumShreds() === 1) {
    ...
    static configureRecorder(audioContext: AudioContext, gainNode: GainNode) {
        ...
        Recorder.recorder.onstop = async () => {
            ...
            const filename = ProjectSystem.activeFile.getFilename().slice(0, -3);
            const now = new Date();
            const date = now.toLocaleDateString().replace(/\//g, "-");
            const time = now.toLocaleTimeString().replace(/:/g, "-").replace(/\s/g, "");
            const dateTime = `${date}_${time}`;

            const blobLink = getBlobLink(wavBlob, filename + "_" + dateTime + ".wav");
            Console.print(`Preview recording at \x1b[38;2;34;178;254m${blobLink}\x1b[0m`);
    ...
    static startRecording() {
        ...
        Recorder.playButton.removeEventListener("click", Recorder.startRecording);
