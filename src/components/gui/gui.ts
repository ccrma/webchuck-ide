...
export default class GUI {
    ...
    static generateGUI(sliderValues?: number[]) {
        ...
        for (let i = 0; i < globals.Event.length; i++) {
            const x =
                (i % GUI.buttonsPerRow) * (BUTTON_SIZE + MARGIN) +
                MARGIN;
            const y =
        ...
        const sliderStartPos = MARGIN + Math.ceil(GUI.buttons.length / GUI.buttonsPerRow) * (BUTTON_SIZE + MARGIN);

        // create a canvas slider for each float
        for (let i = 0; i < globals.float.length; i++) {
            const x =
                (i % SLIDERS_PER_ROW) * (GUI.sliderWidth + MARGIN) +
                MARGIN;
            const y =
